...
- `/about` — 小雀シーガーズとは（準備中）
- `/blog` — ブログ（準備中）。閲覧には管理者とは別のパスワードが必要（`/admin/settings` で変更）
...
- `/players` — 成績ページ（ヘッダー上は「成績」表記）。チーム成績（年度別＋通算）・打撃成績・投手成績のタブ切り替え。
  年度・期間・規定・試合種別（公式戦/練習試合）・大会名・対戦相手のフィルターは URL パラメータ（`?tab=` `?year=` `?gtype=` `?q=0`（規定絞り込みは既定ON） `?tournament=` `?opponent=` など）で共有可能
